...
`EnvConf` / `LocalConf` / `CondaConf` / `DockerConf` / `QlibCondaConf`（均无 env 前缀）。这些类的实例通常在代码里直接构造（如 `DockerConf(image=..., mount_path=..., default_entry=...)`），**不通过 .env 配置**。完整字段见 [API.md §3.6](API.md)。
...
**配套文档**：[API.md](API.md)（接口总览）、[CLAUDE.md §6](../../../CLAUDE.md)（环境策略与 .env 模板原始约定）
