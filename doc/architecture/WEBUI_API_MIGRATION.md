...
> **2026-07-25 状态更新：已接通。** 当前 [`rdagent/log/ui/storage.py`](file:///home/zxh/projects/1.multialphaV/RD-Agent/rdagent/log/ui/storage.py) 已有 `token_cost` 分支，实时推送与历史回放均可生成 token 消息；下方“断点”保留为迁移时的历史诊断，不再代表当前源码。
...
| 4. 实时推送 | ✅ 已接通 | 当前 `_obj_to_json` 的 `token_cost` 分支透传并清洗非有限浮点值 |
| 5. 历史回放 | ✅ 已接通 | `read_trace` 复用同一分支 |
| 6. 前端 | ✅ 已验证 | 最新浏览器 E2E 报告已验证 token 面板显示 |

**历史关键修正**：2026-07-20 调研时，老项目与当时的新项目 `storage.py` 都无 `token_cost` 分支，因此原问题不是版本迁移丢失，而是共同的序列化断链。此后本项目已新增该分支；该结论不能反推 0.8 项目也已同步。
...
#### 迁移建议
- **后端状态**：✅ 已实现。[`ui/storage.py`](file:///home/zxh/projects/1.multialphaV/RD-Agent/rdagent/log/ui/storage.py) 的 `_obj_to_json` 已增加 `token_cost` 分支，直接透传 dict、补 loop_id，并清洗 NaN/Infinity。
...
- msg 序列化（token_cost 已接通）：[`rdagent/log/ui/storage.py`](file:///home/zxh/projects/1.multialphaV/RD-Agent/rdagent/log/ui/storage.py)（`_obj_to_json` 的 `token_cost` 分支）
...
**两项迁移均未移植老项目的 patch 层**——实时日志复用新项目已有的 `/stdout`（+ Range），token 面板使用本项目序列化分支，保持向后兼容。
