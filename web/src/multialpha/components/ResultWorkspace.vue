<template>
  <section class="result-workspace">
    ...
    <div class="result-body">
      ...
      <div v-else-if="tab==='chart'" class="chart-wrap"><iframe v-if="chartUrl" class="center-chart-frame" :src="chartUrl" sandbox="allow-scripts"/><div v-else class="empty-result">当前轮次暂无收益曲线</div></div>
...
import { computed,ref,watch } from 'vue';import { ElMessage } from 'element-plus';import FormulaBlock from './FormulaBlock.vue';import { fetchSota } from '../api';import type { CodeFile,ChartRef,FactorItem,FeedbackSummary,MetricItem,ResultTab } from '../types'
const props=defineProps<{traceId:string;factors:FactorItem[];codes:CodeFile[];chartRef:ChartRef|null;chartHtml:string;metrics:MetricItem[];feedback:FeedbackSummary}>();defineEmits<{download:[]}>();const tab=ref<ResultTab>('conclusion'),codeIndex=ref(0);watch(()=>props.codes,()=>codeIndex.value=0);const currentCode=computed(()=>props.codes[codeIndex.value]);const codeLines=computed(()=>currentCode.value?.content.split('\n').length||0);const hasConclusion=computed(()=>props.metrics.length>0||props.feedback.decision!==null||!!props.feedback.reason);
// C6: chartUrl 懒加载——从 descriptor 的 chart_ref 拼 artifact 端点 URL
const chartUrl=computed(()=>{
  if(props.chartRef){const params=new URLSearchParams({id:props.chartRef.trace_id});if(props.chartRef.loop_id!=null)params.set('loop',String(props.chartRef.loop_id));return `/api/v2/trace/artifact?${params.toString()}`}
  if(props.chartHtml){const blob=new Blob([props.chartHtml],{type:'text/html'});return URL.createObjectURL(blob)}
  return ''
});const tabs=computed(()=>[{label:'最终结论',value:'conclusion' as ResultTab,count:0,available:hasConclusion.value},{label:'因子结果',value:'factors' as ResultTab,count:props.factors.length,available:props.factors.length>0},{label:'收益曲线',value:'chart' as ResultTab,count:0,available:!!chartUrl.value},{label:'因子代码',value:'code' as ResultTab,count:props.codes.length,available:props.codes.length>0}]);watch(tabs,value=>{const current=value.find(item=>item.value===tab.value);if(!current?.available)tab.value=value.find(item=>item.available)?.value||'conclusion'},{immediate:true});const coreMetrics=computed(()=>props.metrics.filter(item=>['IC','年化收益','最大回撤','信息比率'].includes(item.label)).slice(0,4));const feedbackItems=computed(()=>[{icon:'📌',label:'决定理由',value:props.feedback.reason},{icon:'🔍',label:'实验观察',value:props.feedback.observations},{icon:'📊',label:'假设评估',value:props.feedback.evaluation},{icon:'⚠',label:'异常信息',value:props.feedback.exception}].filter(item=>item.value));function formatMetric(item:MetricItem){if(item.rawValue==null)return item.value;if(item.percent)return`${(item.rawValue*100).toFixed(2)}%`;return item.rawValue.toFixed(4)}async function copyCode(){if(!currentCode.value)return;await navigator.clipboard.writeText(currentCode.value.content);ElMessage.success('代码已复制')}function downloadCurrent(){if(!currentCode.value)return;const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([currentCode.value.content],{type:'text/plain'}));link.download=currentCode.value.name;link.click();URL.revokeObjectURL(link.href)}
// SOTA dialog state
const numbers=['一','二','三','四','五','六','七','八','九','十'];const loopLabel=(loop:number|null)=>loop==null?'—':`第${numbers[loop]||loop+1}轮`
