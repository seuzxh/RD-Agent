...
const props=defineProps<{modelValue:boolean}>();const emit=defineEmits<{'update:modelValue':[value:boolean];submit:[value:{method:TaskMethod;description:string;scenario:string;loops:number;modelSelector:string;files:File[]}]}>();const visible=ref(props.modelValue),submitting=ref(false),files=ref<File[]>([]);const form=reactive<{method:TaskMethod;description:string;scenario:string;loops:number;modelSelector:string}>({method:'text',description:'',scenario:'Finance Data Building',loops:10,modelSelector:'lgbm'})
