...
const props=defineProps<{modelValue:boolean}>();const emit=defineEmits<{'update:modelValue':[value:boolean];submit:[value:{method:TaskMethod;description:string;scenario:string;loops:number;files:File[]}]}>();const visible=ref(props.modelValue),submitting=ref(false),files=ref<File[]>([]);const form=reactive<{method:TaskMethod;description:string;scenario:string;loops:number}>({method:'text',description:'',scenario:'Finance Data Building',loops:10})
