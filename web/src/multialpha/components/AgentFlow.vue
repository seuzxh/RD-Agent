...
import { computed,ref } from 'vue';import FormulaBlock from './FormulaBlock.vue';import type { CodeFile,FactorItem,FeedbackSummary,TraceMessage,TraceStatus } from '../types'
const props=defineProps<{messages:TraceMessage[];factors:FactorItem[];codes:CodeFile[];metricValues:Record<string,number|string>;feedback:FeedbackSummary;hypothesis:Record<string,unknown>|null;status?:TraceStatus}>();const active=ref('')
const agents=computed(()=>[{key:'hypothesis',name:'假设生成',role:'研究员',icon:'🧠',done:!!props.hypothesis,stat:''},{key:'design',name:'实验设计',role:'设计师',icon:'✏️',done:!!props.factors.length,stat:props.factors.length?`${props.factors.length} 因子`:''},{key:'coding',name:'代码实现',role:'编码员',icon:'▰',done:!!props.codes.length,stat:props.codes.length?`${props.codes.length} 文件`:''},{key:'backtest',name:'回测执行',role:'执行员',icon:'📊',done:props.metricValues.IC!=null,stat:props.metricValues.IC!=null?`IC=${Number(props.metricValues.IC).toFixed(3)}`:''},{key:'feedback',name:'反馈评审',role:'评审员',icon:'🔍',done:props.feedback.decision!==null,stat:props.feedback.decision===null?'':props.feedback.decision?'已采纳':'已拒绝'}]);const activeAgent=computed(()=>agents.value.find(agent=>agent.key===active.value));const hypothesisText=computed(()=>String(props.hypothesis?.hypothesis||props.hypothesis?.concise_observation||''));const feedbackItems=computed(()=>[{label:'决定理由',value:props.feedback.reason},{label:'实验观察',value:props.feedback.observations},{label:'假设评估',value:props.feedback.evaluation},{label:'下一轮新假设',value:props.feedback.newHypothesis},{label:'异常信息',value:props.feedback.exception}].filter(item=>item.value))
</script>
