...
1. **树搜索无法随计算量扩展**：当分支因子大、每步代价高时（如训练模型、回测因子），MCTS 式的多分支探索会产生组合爆炸，算力浪费严重。
2. **反馈可以充当"梯度"**：实验结果（指标、错误日志）不是简单的"好/坏"标签，而是包含了方向性信息——它告诉智能体下一步应该往哪个方向调整假设或代码。
3. **执行轨迹是宝贵的"动量"**：历史轨迹中积累的成功经验和失败教训，能让后续迭代更快收敛，类似于动量梯度下降避免在局部震荡。
4. **单路径迭代 + 方向性反馈优于多分支搜索**：在 MLE-bench 等真实机器学习工程任务上，梯度式推理比树搜索更高效、更节省 token 和算力。
5. **论文被 ACL 2026 Findings 接收**，相关执行轨迹以 Gome GPT-5 Traces 数据集公开，推动了智能体推理范式的研究。
