...
1. **微调可以被自动化为一个闭环研发问题**：数据集选择、超参配置、训练执行、评估诊断、再迭代，每一步都能由语言智能体自主完成，而不需要人工介入。
2. **Fail-fast 显著降低试错成本**：通过在正式训练前进行烟雾测试，能在分钟级发现数据格式错误、显存不足等问题，避免数小时的无效 GPU 计算。
3. **多级反馈优于单一指标**：仅看最终准确率无法判断下一步该调什么；结合损失曲线、验证集趋势、训练日志进行诊断式反馈，能让智能体做出更精准的调整决策。
4. **经验复用是提升效率的关键**：把成功配置和失败模式沉淀为可检索的经验库，后续任务可以直接借鉴，这与 CoSTEER 的知识图谱思想一致。
5. **FT-Agent 在标准化微调基准上显著优于人工调参和朴素自动调参基线**，证明了"LLM 作为训练工程师"这一范式的可行性。
