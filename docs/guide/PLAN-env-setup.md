...
依据 [CLAUDE.md](../../../CLAUDE.md) §6（环境配置）+ §5（CodeGraph）+ §7（0.8 兼容），为裁剪后的 RD-Agent 代码搭建项目级独立运行环境，使其能在 conda env `multialphav` 中正常加载 Qlib 场景。
...
| 3 | qlib 数据软链 `~/.qlib/qlib_data/cn_data → /home/zxh/qlib_data` | ✅ **已完成**（2026-07-18，用户删除旧 root 拥有的 ~/.qlib/ 后重建，owner=zxh 无需 sudo） |
...
qlib 数据软链已建立：`~/.qlib/qlib_data/cn_data → /home/zxh/qlib_data`。

**执行过程**：用户删除了原 root 拥有的整个 `~/.qlib/` 目录（含停在 2020-09-25 的旧数据 + 未解压的 `20260517011423_qlib_data_cn_1d_latest.zip`），随后由 Agent 重建空目录层级并建立软链（owner=zxh，无需 sudo）。代码里 `provider_uri` 硬编码的 `~/.qlib/qlib_data/cn_data`（6 处 yaml + generate.py）经软链自动指向最新数据 `/home/zxh/qlib_data`。

**验证结果**（0.8 rdagent env）：日历 6430 天，范围 2000-01-04 ~ 2026-07-17；SH600000 收盘价读到 2026-07-17（97.806）。
...
本计划是 [代码裁剪计划](file:///home/zxh/projects/1.multialphaV/.zcode/plans/plan-sess_f11a5a33-68cf-4214-af93-2a6d663f9e78.md) 的后续。裁剪（commit `8f60d6e`）已完成并推送 fork，本计划在其成果上搭建运行环境。两份计划共享同一份 [CLAUDE.md](../../../CLAUDE.md) 权威规范。

---

**版本**：v1.0（2026-07-18 整理）
**适用代码基线**：`cut-non-qlib-scenarios` 分支 @ `8f60d6e`
**权威规范**：[CLAUDE.md](../../../CLAUDE.md) §5/§6/§7/§10
