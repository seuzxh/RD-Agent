...
## 6. 后续修复（2026-07-21 补充）

测试后发现并修复的两个问题：

### 修复 1：SOTA trace_name 匹配失效 ✅（commit `9435b715`）

- **问题**：webUI task 的 FileStorage pickle 在 `<trace_root>/<scenario>/<name>/`，但 `__session__/` 在默认 `log/` 目录（不随 task 变）。`find_session_by_trace_name` 找不到 trace_name 对应的 session → sota 查询 404
- **修复**：`get_sota` 路由增加回退——session 找不到时，`_sota_from_messages()` 从 `/trace` 消息流提取 SOTA（hypothesis + feedback + metric + factors + code）
- **验证**：plain-transformation → `source=message_stream`, sota_loop=2, IC=0.0150, 3 factors

### 修复 2：/stdout 历史任务下载 400 ✅（commit `9435b715`）

- **问题**：`_resolve_stdout_path` 只查 running task（`rdagent_processes`），历史 task 不在内存 → 400
- **修复**：增加 Strategy 2——task 找不到时按 trace_id 拼文件路径 `<trace_root>/<scenario>/<name>.log`，文件存在就返回
- **验证**：plain-transformation → GET 200（1.68MB）+ Range 206（100 字节）
