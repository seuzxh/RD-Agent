...
**已验证通过的修复**（见 [WEBUI_API_MIGRATION.md](../architecture/WEBUI_API_MIGRATION.md)）：
...
- **现象**：`localhost:8081` 前端无法加载任务列表/预测实验，`/traces`、`/predict/experiments` pending/超时。
- **根因**：[web/vite.config.ts#L50-L56](file:///home/zxh/projects/1.multialphaV/RD-Agent/web/vite.config.ts#L50) proxy 指向 `http://115.190.106.124:19899`，本机 curl 该地址超时。
- **状态**：未修复（测试时临时改为 `localhost:19899` 跑通，已恢复原始配置）。
- **优先级**：P0（阻断 dev 环境使用）。

### 🟡 问题 6.2：任务侧边栏状态不准确

- **现象**：除当前正在查看的任务外，其余历史任务均显示「待查看」，无法从列表区分 running/done。
- **根因**：`use-multialpha.ts` 的 `statuses` 仅在 `selectTrace` 时赋值，`loadTraceIds` 不读取各 trace 状态。
- **状态**：未修复。
- **优先级**：P1（体验问题）。

### 🟡 问题 6.3：TokenDashboard 总 TOKEN 显示为 0

- **现象**：任务详情页「总 TOKEN 0」，但「输入 2.6K / 输出 140 / 调用次数 15」。
- **根因**：[trace-model.ts#L45](file:///home/zxh/projects/1.multialphaV/RD-Agent/web/src/multialpha/trace-model.ts#L45) 使用 `accumulated_*` 字段计算 total，但后端 `token_cost` 实际只有 `prompt_tokens` / `completion_tokens`。
- **状态**：未修复。
- **优先级**：P1（展示错误）。

### 🟡 问题 6.4：健康检查弹窗内容缺失

- **现象**：弹窗只显示「⚠️ 存在需要注意的配置项」，未列出 LLM/Docker/Qlib/Conda/MLflow 各检查项。
- **状态**：待进一步确认（弹窗列表渲染或样式问题）。
- **优先级**：P1。

### ✅ 已验证正常（本轮）

- 落地页渲染、品牌一致性、任务详情页全组件、AgentFlow 展开、Loop 切换、ResultWorkspace 各 tab、SOTA 弹窗、股池预测看板、新建任务弹窗校验。
