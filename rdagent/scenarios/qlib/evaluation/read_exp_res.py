"""
Canonical read_exp_res.py — source of truth for both factor_template and model_template.

This script runs INSIDE the Docker container as `python read_exp_res.py`.
It reads the latest mlflow recorder from the qrun execution and exports:
  - qlib_res.csv: structured metrics (IC, ICIR, annualized_return, etc.)
  - ret.pkl: daily return series from portfolio backtest
  - ret_group.pkl: group returns (Group1-5 + long-short)

Both factor_template/read_exp_res.py and model_template/read_exp_res.py
are identical copies of this file. Update this canonical copy first,
then sync to both template directories.
"""
